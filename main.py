...
models = [
    LongformerModel(model_name="allenai/longformer-base-4096", num_labels=num_labels, max_length=4096),
    BigBirdModel(model_name="google/bigbird-roberta-base", num_labels=num_labels, max_length=4096),
    LegalBERTModel(model_name="nlpaueb/legal-bert-base-uncased", num_labels=num_labels)
    ]
...
for model in models:
    ...
    train_dataset = Dataset.from_dict({
        "input_ids": [x["input_ids"] for x in train_X],
        "attention_mask": [x["attention_mask"] for x in train_X],
        "labels": train_y.tolist()
    })

    eval_dataset = Dataset.from_dict({
        "input_ids": [x["input_ids"] for x in test_X],
        "attention_mask": [x["attention_mask"] for x in test_X],
